...
public:
	...
	float		*timebuffer;     /* for benchmarking */
	char* framebuffer;
	...
	void shading_equation(GzCoord normals, GzRender* render, GzColor C, GzTextureIndex uv, bool textured, GzColor gourad_term = NULL);

	// HW1: Display methods
	int GzDefault();
	int GzBeginRender();
	int GzPut(int i, int j, GzIntensity r, GzIntensity g, GzIntensity b, GzIntensity a, GzDepth z);
	int GzPutTime(int i, int j, std::chrono::duration<double, std::nano> d);
